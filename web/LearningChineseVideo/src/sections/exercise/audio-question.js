...
export default function QuestionAudio({ type, setType, lessonId }) {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      audioUrl: Yup.string().required("Audio câu hỏi không được để trống"),
      answers: Yup.array()
  ...
  return (
    <form onSubmit={formik.handleSubmit}>
      <Typography variant="h6">Dạng bài tập nghe để trả lời câu hỏi</Typography>
        ...
        <Box sx={{ width: "82%" }}>
          <UploadAudioFrame
            onUploadedVideo={(data) => {
              console.log(data);
              formik.setFieldValue("audioUrl", data.videoUrl);
