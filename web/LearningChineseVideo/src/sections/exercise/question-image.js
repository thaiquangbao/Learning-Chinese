...
export default function QuestionImage({ type, setType, lessonId }) {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      imageUrl: Yup.string().required("Hình câu hỏi không được để trống"),
      answers: Yup.array()
  ...
  return (
    <form onSubmit={formik.handleSubmit}>
      <Typography variant="h6">Dạng bài tập xem ảnh trả lời câu hỏi</Typography>
      <Stack direction="column" spacing={3} padding="10px" marginTop="10px">
        ...
        <Box sx={{ width: "82%" }}>
          <UploadVideoThumbnail
            onReceiveThumbnail={(imageUrl) => formik.setFieldValue("imageUrl", imageUrl)}
          />
        </Box>
        <Box sx={{ width: "82%" }}>
          <MuiChipsInput
